...
| Pack lifecycle 胶水脚本 | 通过基座 devserver lifecycle 端点装入 / 卸载 | 🟡 | 各 pack `install.py` / `uninstall.py` | 无隔离 devserver 铁证时，不声称 E2E 通过。**2026-07-05 install.py draft 字段透传修复 land**：SH-R1-01 role_slot `node_type`（`aec95f6`）+ ENV-R4-02 provider_req `provider_family`（`d140723`）——白名单原缺这两字段导致 pack 声明被 install 丢弃，现补齐透传进 lifecycle draft；后端 `lifecycleProviderRequirementInput.ProviderFamily` 早已支持，缺口仅在 install.py；均 E2E neuter RED/GREEN 坐实（provider_family：origin/main→None vs 修复→'frappe'）。 |
