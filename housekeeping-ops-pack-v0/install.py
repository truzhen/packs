...
# -*- coding: utf-8 -*-
"""
家政运营 Pack（客户服务全生命周期）—— 装入正在运行的 Truzhen devserver（可加载）。
...
本 pack 无 knowledge/，知识入库步骤自动跳过。

前置：先起 devserver（产品 defaultPlatformPackAssetSeeds 返回 nil，注册表为空）：
  GOWORK=off go run ./backend/cmd/devserver        # 默认 127.0.0.1:18080
然后：
  python3 housekeeping-ops-pack-v0/install.py

幂等：已启用的场景包/角色包/绑定会跳过；同版本 unload→reload 走 reactivate。
"""
import hashlib
import json
import os
...
def main():
    ...
    def do_seed_scene(install_version):
        ...
        draft = {
            ...
            "template_family": manifest.get("template_family", "客户服务全生命周期型"),
            "flow_id": flow_id,
            "role_slots": [{"slot_id": s["slot_id"], "responsibility": s["responsibility"],
                            ...
                            "slice_scope_policy": s.get("slice_scope_policy", "")} for s in role_slots_doc["role_slots"]],
...
def ingest_knowledge_scope(pack_ref, pack_version_ref, scope_ref, items):
    ...
    batch = {
        ...
        "scene_ref": scene_ref, "tags": ["pack 知识库", "家政运营"],
