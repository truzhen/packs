#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""内容运营 Pack 可信 GUI 装入交接：脚本只读 os-14 与 os-07。"""
...
def main():
    parser = argparse.ArgumentParser(description="把内容运营 Pack 装入交给可信 Truzhen GUI。")
    ...
    if enabled_version != version:
        present_owner_install_handoff(args.client_url, pack_ref, version, len(schedule_refs), args.open_gui)
    ...
    ok, reason = wait_for_owner_schedule_states(call, schedule_refs, {"active"}, args.wait_seconds, args.poll_seconds)
    if not ok:
        die("%s；未观察到 os-07 声明计划全部 active" % reason, INSTALL_CONNECTIVITY if reason == "connectivity" else INSTALL_LIFECYCLE_HTTP)
    print("os-14 已证明精确 Pack 版本启用；os-07 已证明声明计划 active。")
