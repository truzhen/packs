...
- `content_production`：只对 Owner 已选择方向生成中文母内容、内部审计稿、已清洗公开候选和人工发布包。短视频请求至少输出标题、开场钩子、30–60 秒口播、分镜表、字幕要点、封面文案和置顶评论；不得用“待补充”占位冒充完整候选。
...
5. 按 Execution Gateway 为当前 `business_skill_id` 选择并提供的 JSON Schema 输出模型候选；Gateway 负责计算候选 ref、内容哈希、泄漏扫描结果和 receipt candidate，并封装为 `output-contract.json` 的最终运行输出。模型不得自铸这些引用或哈希。
