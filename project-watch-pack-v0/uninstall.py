#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""项目关注 Pack —— 正式卸载（从正在运行的 Truzhen devserver）。

只消费 Owner 可信前台 / Base 已签发并外部注入的卸载证明，调用正式 lifecycle
uninstall 端点。脚本不会自行 Prepare / Confirm，不自铸 decision / run / nonce /
evidence，也不会把环境变量伪装成主权裁定。正式卸载会级联停用知识域；历史候选、
业务对象和 Receipt 仍可反查——卸载不删历史。
...
UNINSTALL_ENDPOINT = "/v3/pack-studio/lifecycle/uninstall"
...
def main():
    ...
    code, body = call("POST", UNINSTALL_ENDPOINT, {
        "pack_ref": pack_ref, "owner_ref": OWNER, "reason": "Owner 卸载项目关注 pack",
    ...
    print("   （已产生的候选、业务对象与 03 回执仍可反查——卸载不删历史。）")
