...
### 8.5 优化优先于新增（2026-07-11 Owner 裁定，v4 六仓统一）

能不新增 / 不新写代码就不新写，优先在原有代码 / 脚本 / manifest 上优化调整；能不新增 / 不新写 pack / 目录就不新写，优先在原有 pack 结构上优化调整。新增文件 / 脚本 / pack 前先问：现有资产能否优化、复用、去重或扩展达成同样目的？确需新写导致某模块超 2 万行时，做功能拆分使内部子模块各 ≤ 2 万行即可（历史已超模块 grandfather，不回溯强拆；体量语义权威源 = truzhenos `scripts/check-module-size.sh` + 其 AGENTS.md 硬纪律 29/32）。六仓集合后期统称「Truzhen v4 版本」（权威定义 = truzhenos `NAMING_STANDARD.md`；封棺仓 `truzhenv3` 不属 v4）。

### 8.6 任务图执行纪律

- 默认用一个 Agent 的 Loop 完成简单、单角色、单写入任务；只有角色 / 工具确实不同、存在可安全并行的只读工作、条件分支或需要独立验收时才使用任务图。合并节点不损失职责或验收时必须合并。
- 任务图开始前只定义节点、边、共享状态、验收、预算和停止条件；节点可以是同一 Agent 的阶段、工具调用、独立审阅者或人工闸门，不等于必须启用子智能体。
- 每个节点声明 `reads / writes / output / acceptance`，只写自己拥有的状态字段；交接只传文件、引用、diff、测试和证据摘要，不传整段聊天历史。
- 能由代码、测试结果或显式状态判断的路由必须确定性执行；只有需要语义判断时才交给 AI。
- 只读资产清点、来源核验和意见可以并行；同一仓库 / worktree 同时只有一个写入者。manifest、flow、role、knowledge 和 lifecycle 脚本由主执行节点汇总，审阅节点不得顺手修改。
- 执行者自报不算验收；独立复核使用裁剪后的新上下文并亲自重跑证据。环境不允许独立 Agent 时使用等价的全新上下文对抗复核，不伪称独立 Agent 已验收。
- 每个修复回边必须设最大次数；同一错误连续两次仍未定位立即停止。验收通过、预算耗尽、需要新增授权、范围漂移或外部状态阻断时结束并报告。
...
| `receivable-management-pack-v0/` | 应收管理 Pack（账龄巡检 + 催办候选，只声明不执行）；设计中→已接线（os 侧催办运行时端口经 policy_ref 绑定；待 os 主批合入与 Owner 验收） | 分支 `codex/g4-wc-receivable-pack-packs-20260902` |
