...
## 1. 本仓职责
承载可独立**加载 / 卸载 / 分发**的行业工作台、流程、角色、知识、能力引用（场景荚 = Domain Work Pack）。每个包是一个自包含文件夹：`manifest.json`（六件事声明）+ `flows/`（GateFlowSpec 流程图）+ `role-slots/` + `role-packs/` + `capabilities/` + `knowledge/`（领域知识，权威资料结构化）+ `install.py` / `uninstall.py`（经基座真实 lifecycle 端点装入 / 卸载，若某包尚无卸载脚本必须在 `MODULES.md` 标明）。包**面向 `truzhen-contracts` 契约**编写，物理上 import 不到基座 `truzhenos` 内部。
...
- **禁止把能力实现塞进本仓**：能力荚执行 provider（如 PDF / OpenMontage / code-assistant / software-expert）属于 `truzhenos` 或外部 provider / `truzhen-software`；本仓只声明能力引用 / ProviderRequirement，不写执行代码。
...
- **跨仓路径找不到 pack** → `truzhen-packs` 必须与 `truzhenos` **平级**放在同一父目录（基座加载器 `runtime.Caller` 上溯到五落点父目录再进 `truzhen-packs`）。
