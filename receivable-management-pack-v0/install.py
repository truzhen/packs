...
# -*- coding: utf-8 -*-
"""
应收管理 Pack（账龄巡检与催办候选）—— 装入正在运行的 Truzhen devserver（可加载）。
...
本 pack 的 knowledge/dunning（中性话术模板 / 禁用词表 / 作业口径 SOP）随装入入库，
全部为合成内容，不含真实客户、金额或人名。

Pack 只装声明：账龄事实来源、催办策略、发送裁定分别在 os 侧读模型、沟通网关与
Owner + Base Gate；本脚本不发送任何催办、不写回任何单据。

前置：先起 devserver：
  GOWORK=off go run ./backend/cmd/devserver        # 默认 127.0.0.1:18080
然后：
  python3 receivable-management-pack-v0/install.py
...
def main():
    ...
    def do_seed_scene(install_version):
        ...
        draft = {
            ...
            "template_family": manifest.get("template_family", "应收账款催办治理型"),
            "flow_id": flow_id,
...
def ingest_knowledge_scope(pack_ref, pack_version_ref, scope_ref, items):
    ...
    batch = {
        ...
        "scene_ref": scene_ref, "tags": ["pack 知识库", "应收管理"],
