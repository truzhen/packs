...
# -*- coding: utf-8 -*-
"""
应收管理 Pack（账龄巡检与催办候选）—— 从正在运行的 Truzhen devserver 卸载（停用）。

走真实 lifecycle/disable 端点：先经 Base gated-action prepare→confirm 取得真签发的
decision_ref/run_id/nonce（禁自铸），再 disable。停用只停当前版本并收回
KnowledgeMount 可见性；已产生的事务对象、候选和回执在 03 仍可反查，停用不删除历史。

用法：
  TRUZHEN_DEVSERVER_BASE=http://127.0.0.1:18080 python3 receivable-management-pack-v0/uninstall.py

停用是 Owner + Base Gate 的高风险动作。对要求 Owner presence 的运行端，调用方须
从已登录的本地产品会话注入（值不打印、不写盘）：
  TRUZHEN_OWNER_PRESENCE_COOKIE=<server-issued-value>
可选的 `TRUZHEN_OWNER_ORIGIN` 默认为 `http://tauri.localhost`。脚本不生成、不伪造
此证明；缺失时 fail-closed。
"""
import json
import os
...
OWNER_PRESENCE_COOKIE = os.environ.get("TRUZHEN_OWNER_PRESENCE_COOKIE", "").strip()
OWNER_ORIGIN = os.environ.get("TRUZHEN_OWNER_ORIGIN", "http://tauri.localhost").strip()
...
def call(method, path, body=None):
    ...
    # 01 Base 的高风险 Gate 只接受受信任 Owner Origin 与 server-issued
    # HttpOnly presence proof。Pack 只转发由已登录产品会话显式交给它的证明，绝不自铸。
    if path.startswith("/v3/base/gated-actions/"):
        req.add_header("Origin", OWNER_ORIGIN)
        if OWNER_PRESENCE_COOKIE:
            req.add_header("Cookie", "truzhen_owner_presence=" + OWNER_PRESENCE_COOKIE)
    try:
...
def main():
    ...
    pack_name = manifest.get("name", "应收管理 Pack")
    ...
    code, body = call("POST", "/v3/base/gated-actions/prepare", {
        ...
        "target_ref": pack_ref,
        "content_summary": "停用应收管理 Pack：" + pack_ref,
        "impact_summary": "停用应收催办场景包并收回其知识域挂载；历史事务对象、候选和回执保留可反查",
