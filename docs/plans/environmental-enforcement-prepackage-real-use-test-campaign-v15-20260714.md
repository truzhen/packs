...
| 最小可交付 | 同一 Pack v1.0.0、同一 digest、同一安装实例、同一 `transaction_ref` 完成可回放 lifecycle；通过 manifest/schema、Candidate/Formal、最小权限、Gate/Gateway/Receipt、幂等重试、断网恢复与卸载隔离硬门；PDF 前显示可解释阻断；PDF 后事实、法源版本、辖区和证据引用均可追溯，未知项全部标 `pending_human_review`。 |
...
测试执行者已获本计划范围内全部本地权限：建立隔离 worktree、专用 HOME/SQLite/浏览器 profile，启停 OS/client，本地真实模型调用，使用合成 PDF，直接安装/停用/启用/卸载/重装，截图、只读 API/SQLite/Receipt 对账，以及修复不改契约/主权链的中小问题。不得触发真实市场、付款、正式签发、送达、删除、对外提交、执法执行、生产 Frappe 或客户数据。

隔离分级必须写入每条测试证据：纯知识与 manifest 声明按 L0 只读校验；角色 Agent 推理按 L1 sandbox，不能持真实执行凭据；OCR、网络、本地软件或其它外部能力按 L2 隔离 Provider，经 Gateway 调用并留 Receipt。容器、进程或端口“启动成功”只证明载体存活，不能替代 readiness、Gate、Gateway、Receipt 或业务结果验收。
...
1. `manifest.json`、flow、role、capabilities、knowledge scope/index 必须通过权威 schema 与结构校验；不得在 Pack 内复制、放宽或私造 contracts schema，也不得以解析成功代替语义校验。
2. Candidate 与 Formal 必须物理、接口和 UI 三层隔离。Role Pack、模型、Pack flow 只能产生 Candidate；Formal 只能由具备资格的人确认并通过 Owner + Base Gate 产生。候选 ref、`receipt_candidate://` 或草稿不得显示为正式裁定、正式文书或正式回执。
3. 权限取 Owner 授权、角色白名单、Pack 声明、ContextSlice、KnowledgeMount 与 Provider 能力的最小交集。Pack 不得申请通配权限、持凭据、直连文件系统/网络/外部软件，也不得扩大 Base 已授予的权限。
4. 缺配置、缺证据、缺 Provider、Provider 未 ready、法源/Pack/软件版本冲突时，必须明确返回 `blocked`、`not_ready` 或 `provider_missing`，并给出稳定 reason/missing fields；版本冲突统一为 `blocked(reason=version_conflict)`。不得静默降级、换 Provider、吞错、补假数据或返回空成功；`degraded` 不得用于掩盖上述四类硬缺口。
5. 每个高风险节点必须证明：Candidate → 具备资格的人确认 → Owner/Base Gate 裁定；真实能力只能由对应 Gateway 调用。不得由 Pack、前端、Provider readiness 或容器状态代替 Gate，不得绕过 Gateway 直调 OCR、模型、网络或本地软件。
6. 每个被接受、阻断和执行尝试都要有可反查、可回放、可与 decision/candidate/evidence 关联的 03 Receipt；Receipt 回放必须复现当时版本、输入摘要、Gate 结论和副作用计数，不能从当前状态重算后冒充历史。
7. 安装、启停、节点推进、重试、重复点击、旧请求重放和卸载必须使用稳定幂等键。断网前已提交/未提交边界要可判定，恢复后不得重复 Candidate、Formal、Receipt 或外部动作。
8. 卸载后本 Pack 的 RoleBinding、KnowledgeMount、Provider session 与临时资源不可再运行；同一项目历史和 Receipt 可只读回放。不得删除正式历史，也不得与其它 Pack 的项目、知识、候选、Provider session、端口、缓存或资源串包。
9. 对基座做静态防腐审计：`truzhenos` 不得出现 environmental/pack ref/flow node 专用分支、专用 Gate/Gateway/Receipt、行业 schema 副本或 Pack seed 回潮；client 不得成为真相源；本仓不得包含 Provider 实现。若只能靠 Pack 专用基座代码跑通，登记跨仓大问题并停止该 lane，不得现场焊死。
...
4. 对每条法律候选逐项核验法源名称、发布机关、版本/修订状态、生效与失效时间、适用辖区、条款号、09 `knowledge_ref` 和引用证据。法源过期、辖区不匹配、版本冲突或无法证明现行有效时必须 `blocked/not_ready`，只允许输出待具备资格的人核验的 Candidate。
5. 对材料完整性建立必需材料清单，逐项标已提供/缺失/冲突/待核验；缺关键证据不能越过 evidence/compare Gate。截图、PDF、日志和回执中的姓名、电话、证件号、地址及其它敏感信息必须按最小展示脱敏，原件仅留在授权证据目录。
6. 文书草稿与正式签发必须分区、分 ref、分状态展示。处罚、送达、删除、对外提交均为红色动作：默认只做模拟或 Candidate；必须由具备资格的人明确确认并通过 Owner + Base Gate，测试任务不得代签、代送、代删或代提交。
7. 主动投喂一次会诱导编造的合成提示。若模型不遵守严格格式，系统必须 `role_advice_grounding_violation` fail-closed；不能为了“跑通”降级 validator、拼模板或接受自然语言。
8. 按 flow 节点表逐项覆盖 19 节点：`intake`、`screening`、`inspect_prep`、`credentials`、`onsite`、`scan`、`evidence_chain`、`evidence_review`、`fact_advice`、`legal_advice`、`lawyer_challenge`、`compare`、`disposition_shadow`、`disposition`、`correction_task`、`doc_draft`、`owner_gate`、`archive`、`done`。每项标 `PASS / 受控阻断 / not_ready / not_tested`，不得因后段高风险动作不执行就漏记节点。
9. `compare` 及后段只验证 Candidate、诚实 Provider 状态和 Gate 阻断，不作 Owner 正式批准；保持 0 FormalTask、0 文书外发、0 执法执行、0 生产写回。
...
## 8. R3：卸载、历史查询、重装和作者/Provider 覆盖
...
独立验收主体确认：开放 P0=0、阻塞 P1=0；共通治理硬门全部有证据；PDF 前可解释零副作用；PDF 后无越界事实；法源版本/辖区/证据/材料完整性/脱敏通过；文书草稿与正式签发隔离；不合格模型输出 fail-closed；单项目安装→停用→启用→卸载→重装和历史查询全程通过；全部安全可继续 lane 已跑完。达到后才标`已验收（打包前）`；四个市场阶段始终为 `not_tested_owner_directed`。
