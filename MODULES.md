...
|---|---|---|---|---|
| `environmental-enforcement-pack-v0/` | `scene_pack://environmental-enforcement-flow` | 合规审查执法证据链型 | 完整文件夹包；PDF/材料硬门、事实锚定与可逆 lifecycle 修复为`已实现 -> 已接线`，待 v17 从最新主线单项目完整复验（未发布） | 生态环境执法证据链：线索、立案、取证、证据三性、执法精英 / 挑剔律师对照、处置、文书、Owner/Base 裁定、Receipt。 |
| `smart-home-owner-pack-v0/` | `scene_pack://smart-home-owner-project-ops` | 长周期项目交付型 | v1.1.0 完整文件夹包；v16 已真实 GUI 验收，未合并资产收口后按 v17 从最新主线回归；可选 Home Assistant 不作为项目主链放行前提（未发布） | 智能家居老板项目经营：商机、立项、进度、物料、交付、历史查询、Frappe 受控写回；硬件仅可选复用 Home Assistant L2 Provider，不自造程序且不作为项目主链放行前提。 |
...
| `content-operations-workbench-v0/` | `scene_pack://content-operations-workbench` | Founder 自营内容候选与复盘型 | v0.1.0 第二阶段最小闭环`已验收（任务分支，未发布）`：业务 Skill/输出契约/模型 Schema 均由三文件 hash 锁定的 Pack bundle 持有；通用 Host Codex → 本地 08 35B → `PackSkillCandidate`、逐次 T06、动态 Gate 与 os-03 final/wrapper Receipt 已实跑；无平台登录、上传或发布 | 把真实产品证据变成方向候选、母内容、渠道候选、人工发布包与周复盘；Pack 不含 CLI / Provider / 平台登录 / 自动发布，Codex Hands 由 software + 11 Gateway 供给。 |
