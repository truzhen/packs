...
|---|---|---|---|---|
| `environmental-enforcement-pack-v0/` | `scene_pack://environmental-enforcement-flow` | 合规审查执法证据链型 | 完整文件夹包；材料硬门、事实锚定、检索相关性与 Flow/Transaction 完成语义为`已接线`；installer 以 exact enabled pointer + 15 个 required active mount + FormalReceipt 反查作为组合 readiness，之后仍只输出可信 GUI Owner 待办，不自铸 evidence 或确认角色/槽位/知识，待独立复验（未发布） | 生态环境执法证据链：线索、立案、取证、证据三性、执法精英 / 挑剔律师对照、处置、文书、Owner/Base 裁定、Receipt。 |
...
| `inquiry-management-pack-v0/` | `scene_pack://inquiry-management` | 长周期项目交付型（12 族固定目录内取最贴近者；建议新增「线索到报价客户获取型」需 14 制作台矩阵扩展） | v0.1.0 声明骨架，生命周期`设计中`：manifest、flow、单角色槽 / 角色包、能力引用、install / uninstall 与离线契约测试齐备；**无知识库**，Provider 未接通，无隔离 devserver E2E、无真实模型草稿、无前端验收 | 零售客户获取链段 1-4：询盘对象化、线索池扫描、价值分诊、跟进草稿、报价单候选；客户 / 线索 / 报价单真相在外部 ERP，Pack 只持受控快照引用；生成合同交接项目关注 Pack，回款开票归应收 Pack。 |
| `receivable-management-pack-v0/` | `scene_pack://receivable-management` | 应收账款催办治理型 | v0.1.0；只声明不执行，lifecycle 设计中 | 应收管理 Domain Work Pack：账龄巡检 + 催办候选。 |
...
| `install.py` | 暂存允许的 lifecycle candidate、读取 readiness 或输出可信 GUI handoff | 胶水脚本不得自铸 evidence/decision，不得代 Owner 确认；正式状态归基座 |
| `uninstall.py` | 交给可信 GUI 由 Owner 显式停用，并只读等待 os-14 权威状态 | 胶水脚本不伪造 Origin/Cookie/Owner presence，不后台确认 Gate；不物理删除 FormalKnowledge、不破坏历史 Receipt |
...
- `flows/inquiry-management-flow.flow.json`：19 节点 / 19 边，节点 type 全部取自 14 制作台固定注册词表（`packstudio/nodeinfo/nodeinfo.go`）；四道显式 Owner/Base 门（`inquiry_object_confirm` / `triage_confirm` / `followup_send_confirm` / `quotation_confirm`）、两个回执节点、一个向项目关注 Pack 的交接终点；每个能力节点显式声明 04 契约字段 `capability_domain` + `capability_operation`。词表无发送类节点，真实发送由 10 ControlledExternalSend 在 Owner 门后执行并落 03 回执，flow 不自带发送节点。
...
- 报价抬头按 Owner 第 15 条已裁（2026-09-02）写死：`quotation_to` = 服务主体（业主），业主为线索时取线索、已成客户时取业主客户；来源渠道客户不得作 `quotation_to` / `party_name`，无可配置策略。
