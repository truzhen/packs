...
| `housekeeping-ops-pack-v0/` | `pack_id: pack_housekeeping_ops_v0`（旧 schema，待升级为 `pack_ref` + 补 `template_family` + `install.py` + `knowledge/`） | （缺，待补） | 早期（声明 + `seed-into-running-devserver.sh`，无 install.py / knowledge 目录） | 家政公司客户服务全生命周期：受理咨询→顾问出方案→质检质询→排期报价→派工任务主权确认→上门执行→归档。 |
| `templates/` | — | — | 脚手架 | 文件夹包模板（如 `scene-pack-software-template`），不参与分发。 |

> **待统一项（记入交接）**：`housekeeping-ops-pack-v0` 用早期 schema（`pack_id` 而非 `pack_ref`、缺 `template_family`、无 `install.py` / `knowledge/`）。env / smart-home 已对齐新 schema。后续应升级 housekeeping 对齐（不破坏其 Go e2e 测试）。
...
| `install.py` / `uninstall.py` | 经基座真实 lifecycle 端点装入 / 卸载（canvas→draft→readiness→promote→confirm→role-pack→knowledge）。 |
