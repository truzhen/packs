...
|---|---|---|---|---|
| `environmental-enforcement-pack-v0/` | `scene_pack://environmental-enforcement-flow` | 合规审查执法证据链型 | 完整文件夹包；PDF 前可解释 EvidenceGap 与 PDF 后模型输出事实锚定均`已实现 -> 已接线`，按 v15 单项目完整 lifecycle 计划待真实 GUI `已验收`（未发布） | 生态环境执法证据链：线索、立案、取证、证据三性、执法精英 / 挑剔律师对照、处置、文书、Owner/Base 裁定、Receipt。 |
| `smart-home-owner-pack-v0/` | `scene_pack://smart-home-owner-project-ops` | 长周期项目交付型 | v1.1.0 完整文件夹包；单项目五阶段声明与可选 Home Assistant ProviderRequirement `已实现 -> 已接线`，待 v16 真实 GUI 独立验收（未发布） | 智能家居老板项目经营：商机、立项、进度、物料、交付、历史查询、Frappe 受控写回；硬件仅可选复用 Home Assistant L2 Provider，不自造程序且不作为项目主链放行前提。 |
...
| `shuxuejia-renovation-pack-v0/` | `scene_pack://shuxuejia-large-home-renovation` | 长周期项目交付型 | 新增完整文件夹包：manifest、457 节点 / 543 边 EPC flow、7 角色包、capabilities、knowledge、install、uninstall；待真实运行和前端用户视角验收 | 墅学家大宅装修设计指导：设计准备、深化设计、材料合同付款、现场质量验收、售后保修，多角色候选协作，Owner/Base Gate 和 Receipt 回放。 |
