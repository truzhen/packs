...
|---|---|---|---|---|
| `environmental-enforcement-pack-v0/` | `scene_pack://environmental-enforcement-flow` | 合规审查执法证据链型 | 完整文件夹包；材料硬门、事实锚定、检索相关性与 Flow/Transaction 完成语义为`已实现 -> 已接线`；installer 以 exact enabled pointer + 15 个 required active mount + FormalReceipt 反查作为组合 readiness，之后仍只输出可信 GUI Owner 待办，不自铸 evidence 或确认角色/槽位/知识，待独立复验（未发布） | 生态环境执法证据链：线索、立案、取证、证据三性、执法精英 / 挑剔律师对照、处置、文书、Owner/Base 裁定、Receipt。 |
