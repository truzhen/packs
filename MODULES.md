...
|---|---|---|---|---|
| `environmental-enforcement-pack-v0/` | `scene_pack://environmental-enforcement-flow` | 合规审查执法证据链型 | 完整文件夹包；v10 修复`已实现`，待单 Pack 三轮真实 GUI `已验收`（未发布） | 生态环境执法证据链：线索、立案、取证、证据三性、执法精英 / 挑剔律师对照、处置、文书、Owner/Base 裁定、Receipt。 |
| `smart-home-owner-pack-v0/` | `scene_pack://smart-home-owner-project-ops` | 长周期项目交付型 | 完整文件夹包；v10 急停修复`已实现`，待单 Pack 三轮真实 GUI `已验收`（未发布） | 智能家居老板项目经营：机会、Frappe 项目 / 客户快照、项目经理建议、里程碑 / 采购 / 施工候选、客户沟通、Frappe 写回候选、项目回执。 |
