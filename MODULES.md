...
| `content-operations-workbench-v0/` | `scene_pack://content-operations-workbench` | Founder 自营内容候选与复盘型 | v0.1.1 已完成打包前验收：业务 Skill、输出契约和按三种工作模式选择的模型 Schema 均由 framed hash 锁定；真实 GUI 已生成完整 45 秒抖音候选，并可反查 T06、动态 Gate、08 usage、11 执行与 03 Receipt；代码发布不等于产品已安装、启用或上架 | 把真实产品证据变成方向候选、母内容、渠道候选、人工发布包与周复盘；Pack 不含 CLI / Provider / 平台登录 / 自动发布，Codex Hands 由 software + 11 Gateway 供给。 |
