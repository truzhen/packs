...
# -*- coding: utf-8 -*-
"""
墅学家大宅装修设计指导 Pack —— 装入正在运行的 Truzhen devserver（可加载）。
...
前置：先在隔离基座 worktree 起 devserver，并通过 TRUZHEN_PACKS_ROOT 指向本 packs worktree：
  go run ./backend/cmd/devserver        # 默认 127.0.0.1:18080
然后：
  python3 packs/shuxuejia-renovation-pack-v0/install.py
...
def ingest_knowledge_scope(pack_ref, pack_version_ref, scope_ref, items):
    ...
    batch = {
        ...
        "scene_ref": scene_ref, "tags": ["pack 知识库", "墅学家大宅装修设计指导"],
    ...
    for c in candidates:
        ...
        code, abody = call("POST", "/v3/memory/knowledge/candidates/" + urllib.parse.quote(cref, safe="") + "/approve", {
            ...
            # 装入 pack = Owner 同意挂载知识，但不等于逐条人工核验；正式知识保持待复核。
            "verify_authority": False,
            "reason": "随场景包装入并经 Owner 同意导入，保持 pending_human_review，待人工核验"})
        if code != 200:
